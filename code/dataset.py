...
class LRW_1D_lstm_landmark_pca(data.Dataset):
    def __init__(self,
        ...
        self.train = train
        self.num_frames = 16  
        self.lmark_root_path = '/mnt/ssd0/dat/lchen63/lrw/data/landmark1d'
        ...
        #     transforms.Normalize(mean=(0.0), std=(0.2))
        # ])
    ...
    def __getitem__(self, index):
        if self.train=='train':
            ...
            mfcc_path = os.path.join('/mnt/ssd0/dat/lchen63/lrw/data/mfcc/',  self.train_data[index][0],  self.train_data[index][1],  self.train_data[index][2] + '.npy') 
        ...
        if self.train=='test':
            ...
            mfcc_path = os.path.join('/mnt/ssd0/dat/lchen63/lrw/data/mfcc/',  self.test_data[index][0],  self.test_data[index][1],  self.test_data[index][2] + '.npy') 
...
class LRW_1D_single_landmark_pca(data.Dataset):
    def __init__(self,
        ...
        self.num_frames = 16  
        self.lmark_root_path = '/mnt/ssd0/dat/lchen63/lrw/data/landmark1d'
        self.audio_root_path = '/mnt/ssd0/dat/lchen63/lrw/data/audio'
        self.pca = torch.FloatTensor(np.load('../basics/U_lrw1.npy')[:,:6] ) 
        self.mean = torch.FloatTensor(np.load('../basics/mean_lrw1.npy'))
...
class LRWdataset1D_single_gt(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.train=='train':
                ...
                #load righ img
                image_path = '/mnt/ssd0/dat/lchen63/lrw/data/regions/' +  self.train_data[index][0]
                landmark_path = '/mnt/ssd0/dat/lchen63/lrw/data/landmark1d/' + self.train_data[index][0][:-8] + '.npy'

                landmark = np.load(landmark_path) * 5.0

                right_landmark = landmark[self.train_data[index][1] - 1]
                # tp = (MS + np.dot(right_landmark.reshape(1,6), EIGVECS))
            ...
            # try:
                #load righ img
            image_path = '/mnt/ssd0/dat/lchen63/lrw/data/regions/' +  self.test_data[index][0]
            landmark_path = '/mnt/ssd0/dat/lchen63/lrw/data/landmark1d/' + self.test_data[index][0][:-8] + '.npy'
            landmark = np.load(landmark_path) * 5.0
            ...
            # r = random.choice(
            #     [x for x in range(1,30)])
            # example_path =   image_path[:-8] + '_%03d.jpg'%2
            # example_landmark = landmark[1]
    ...
    def __getitem__(self, index):
        ...
        if self.train=='train':
                ...
                for jj in range(16):
                    ...
                    image_path =  '/mnt/ssd0/dat/lchen63/lrw/data/regions/' +  self.train_data[index][0][:-7] + '%03d.jpg'%this_frame
                    im = cv2.imread(image_path)
                    im = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)
                    im = cv2.resize(im, self.output_shape)
                    im = self.transform(im)
            ...
            #load righ img
                ...
                for jj in range(16):
                    this_frame = current_frame_id + jj
                    image_path =  '/mnt/ssd0/dat/lchen63/lrw/data/regions/' +  self.test_data[index][0][:-7] + '%03d.jpg'%this_frame
                    im = cv2.imread(image_path)
                    im = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)
                    im = cv2.resize(im, self.output_shape)
                    im = self.transform(im)
                ...
                example_path =   image_path[:-8] + '_%03d.jpg'%r
                example_landmark = landmark[r - 1]
                # example_path =   '../image/tcd00001_region.jpg'
                # example_landmark = np.load('../image/tcd00001.npy')
        ...
        #     # try:
        #         #load righ img
        #     right_imgs = torch.FloatTensor(16,3,self.output_shape[0],self.output_shape[1])
        #     right_landmark = torch.FloatTensor(16,136)
        #     for gg in range(16):
        #         this = self.demo_data[index][1] + gg
        #         image_path = '/mnt/disk1/dat/lchen63/lrw/data/regions/' +  self.test_data[index][0]
        #         landmark_path = '/mnt/disk1/dat/lchen63/lrw/data/landmark1d/' + self.test_data[index][0][:-8] + '.npy'
        #         right_lmark = np.load(landmark_path)
        #         right_lmark = torch.FloatTensor(right_lmark.reshape(-1))
        #     # print ('=========================')
        #         # print ('real path: ' +  image_path)
        #         im = cv2.imread(image_path)
        #         if im is None:
        ...
        #         im = self.transform(im)
        #         right_img = torch.FloatTensor(im)
        #         right_imgs[gg,:,:,:] = right_img
        #         right_landmark[gg,:] = right_lmark
        ...
        #     return example_img, example_landmark, right_imgs,right_landmark
    ...
    def __getitem__(self, index):
        ...
        if self.train=='train':
            try:
                ...
                example_landmark =lmark[r,:]
                example_mfcc = mfcc[(r -3) * 4 : (r + 4) * 4, 1 :]
                # example_audio = audio[r * 670 : (r + 1)*670]
               
                # audio = audio[(r + 1)*670: (r + 17)*670]
                mfccs = []
                for ind in range(1,17):
                    t_mfcc =mfcc[(r + ind - 3)*4: (r + ind + 4)*4, 1:]
                    t_mfcc = torch.FloatTensor(t_mfcc)
